...
public partial class Plugin {
  [ConsoleCommand("timeleft",
    "Prints in the chat the timeleft in the current map")]
...
public class TimeLeftCommand : IPluginDependency<Plugin, Config> {
  ...
  public TimeLeftCommand(TimeLimitManager timeLimitManager,
    MaxRoundsManager maxRoundsManager, GameRules gameRules,
  ...
  public void OnConfigParsed(Config config) {
    _config = config.Timeleft ?? new TimeleftConfig();
  }
  ...
  public void CommandHandler(CCSPlayerController? player) {
    ...
    if (!_timeLimitManager.UnlimitedTime) {
      if (_timeLimitManager.TimeRemaining > 1) {
        var remaining =
          TimeSpan.FromSeconds((double)_timeLimitManager.TimeRemaining);
        if (remaining.Hours > 0)
          text = _localizer.LocalizeWithPrefix("timeleft.remaining-time-hour",
            remaining.Hours.ToString("00"), remaining.Minutes.ToString("00"),
            remaining.Seconds.ToString("00"));
        else if (remaining.Minutes > 0)
          text = _localizer.LocalizeWithPrefix("timeleft.remaining-time-minute",
            remaining.Minutes, remaining.Seconds);
        else
          text = _localizer.LocalizeWithPrefix("timeleft.remaining-time-second",
            remaining.Seconds);
      } else { text = _localizer.LocalizeWithPrefix("timeleft.time-over"); }
    } else if (!_maxRoundsManager.UnlimitedRounds) {
      if (_maxRoundsManager.RemainingRounds > 1)
        text = _localizer.LocalizeWithPrefix("timeleft.remaining-rounds",
          _maxRoundsManager.RemainingRounds);
      else
        text = _localizer.LocalizeWithPrefix("timeleft.last-round");
    } else { text = _localizer.LocalizeWithPrefix("timeleft.no-time-limit"); }
